...
2.  **Serving.** Given preserved storage, does the model still serve old tasks? We decompose the serving problem into *joint* (all neurons active) and *routed* (prefix-masked) regimes, and show the degradation in the former is 83% arithmetic (reproduced by random untrained blocks) while the latter is exact.
...
**Cross-script generalization (Section <a href="#sec:xscript" data-reference-type="ref" data-reference="sec:xscript">8</a>).** The address geometry is byte-statistical, not linguistic: Chinese, Japanese, Hindi, and Inuktitut route 37–40/40 to the only two high-byte territories (Cyrillic, Greek). Chinese acquires from scratch at 2.69 ppl; a Devanagari phase grown on Chinese retains both bit-exactly and serves both at acquisition quality. Monotonic model growth holds across script universes.
...
Hardware: one RTX 4090 (24 GB) and one NVIDIA GB10 (gx10, 121 GB); scale is $`\sim`$<!-- -->100M parameters at the base width ($`\times`$<!-- -->128) and $`\sim`$<!-- -->579M at the final ladder width ($`\times`$<!-- -->736). Every number in this paper traces to a committed artifact; pre-registrations, verdicts, and corrections are append-only records on the project’s message bus, cited by envelope number.
...
## The random-expansion control: 83% is arithmetic
