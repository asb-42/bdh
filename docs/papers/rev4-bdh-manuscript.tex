...
difference, not instrumentation (Section~\ref{sec:open}). The instrument offset once
invoked to excuse a wider spread does not exist at that size---sixteen of the twenty
twenty-by-twenty matrix diagonal cells are, by construction, the logged acquisition
exits themselves; the four measured under genuinely different instruments (pl, fr, es,
en) agree to within $1\%$.
...
eras, so the mechanism is not an English artefact. The Greek era overshoots---a random
block costs $186.8$ where the trained ladder costs $63.7$---the first case in these data
where learned growth was protective relative to noise. That is a new measurement and is
reported as one (Section~\ref{sec:open}); the 83\,\%/62\,\% headline above is specific to
the English-era control.
...
$+4.3\%$, worst case $+8.0\%$ at hu; the routed-cost band and its instrument are measured
in Section~\ref{sec:ra2b}).
...
\textbf{Results we did not explain.} (i)~\emph{The Greek-era overshoot.} Repeating the
random-expansion control across seven era checkpoints (Section~\ref{sec:readout}) finds
every fraction except one below the English value: $f_{\log}$ spans $0.599$ (sl) to
$1.472$ (el) around a median of $0.880$, and the Greek era is the only case where a
\emph{random} block costs more than the \emph{trained} ladder ($186.8$ against $63.7$).
Learned growth in that era was therefore protective relative to noise---the opposite of
the pattern the control was built to expose. We report it as a measured anomaly with no
mechanism attached. A dose-response sweep per era (one width target was measured here,
not the phase-1 six-point curve) is a separate pre-registered experiment.
