...
\begin{abstract}
We study continual learning in \BDH{}, a byte-level depth-recurrent language model whose additive growth rule appends zero-initialized neurons while leaving all existing computation bit-identical. Our central finding separates three concerns that the continual-learning literature usually conflates.
...
\emph{Serving} splits into two measured regimes: joint serving degrades because the summation readout accumulates contributions from all neurons (83\% of the damage is reproduced by appending a single random, untrained block), while prefix-masked serving reproduces acquisition quality exactly (retention equals acquisition, zero drift across a full growth phase, 800/800 routing accuracy).
\emph{Addressing} is the open problem: label-free likelihood selection achieves 20/20 correct prefix identification at every calibration budget from 4\,KB to 1\,MB, and a byte-level $n$-gram addresser reproduces the likelihood router perfectly ($1.000$, Wilson $[0.977,1.000]$) given $\sim$4--8\,KB of calibration text per domain---but cumulative-prefix NLL is not unimodal, so selection is robust yet not sublinear in territory count. Out-of-support detection requires two axes (relative advantage and absolute perplexity): byte-adjacent unseen languages have ratio $\approx 1$ while cross-script inputs have catastrophic joint perplexity that inflates the ratio past the trained floor. Cross-script probes confirm that the address geometry is byte-statistical, not linguistic: Chinese, Japanese, Hindi, and Inuktitut route 37--40/40 to the two territories with by far the densest multi-byte training exposure (Cyrillic and Greek; Latin-script territories carry only sparse diacritic ranges), with zero linguistic kinship. A 20-phase fixed-capacity baseline shows that without growth, forgetting is destruction (not access loss). The growth mechanism generalizes across script universes: a Chinese territory grown on an English base, then Devanagari grown on Chinese, retains all three bit-exactly, routes perfectly, and serves each at acquisition quality.
\end{abstract}
...
\begin{enumerate}
\item \textbf{Storage.} Under masked growth, is previously acquired computation physically preserved? We answer at the bit level: yes, four independent confirmations, including cross-script.
\item \textbf{Serving.} Given preserved storage, does the model still serve old tasks? We decompose the serving problem into \emph{joint} (all neurons active) and \emph{routed} (prefix-masked) regimes, and show the degradation in the former is 83\% arithmetic (reproduced by random untrained blocks) while the latter is exact.
...
\begin{quote}\itshape
\BDH{} provides an append-only neural substrate in which previously acquired computation remains physically intact while new computation is added. The central research problem is stable, efficient, domain-agnostic \textbf{addressing} of the accumulated territories---not destructive forgetting.
\end{quote}
...
\textbf{Cross-script generalization (Section~\ref{sec:xscript}).} The address geometry is byte-statistical, not linguistic: Chinese, Japanese, Hindi, and Inuktitut route 37--40/40 to the only two high-byte territories (Cyrillic, Greek). Chinese acquires from scratch at 2.69 ppl; a Devanagari phase grown on Chinese retains both bit-exactly and serves both at acquisition quality. Monotonic model growth holds across script universes.
...
trainable parameters (its only state is the RoPE frequency buffer), which matters throughout:
...
All experiments use byte-level \BDH{}: the vocabulary is the 256 byte values, sequences are
...
($\times$736). Every number in this paper traces to a committed artifact; pre-registrations,
verdicts, and corrections are append-only records on the project's message bus, cited by
envelope number.
...
\textbf{Mechanism (\textsc{derived}).} AdamW's decoupled weight decay multiplies every
...
numbers below (the RA2b chain, cross-script stages) run under this repair.
...
production MoE training (Marin project tracker, thread 8818), suggesting a failure class,
...
random-expansion control measures it: one random block costs $4.1\times$, and 83\% of the
...
\caption{RA2b final checkpoint: routed serving (blue) vs joint serving (orange) vs acquisition exit (black tick), per domain, log scale. Routed tracks acquisition for every domain (median ratio 1.09, range 1.02--1.13, within the window-vs-val instrument offset); joint serving erodes 2--13$\times$ --- the interference term that survives the fix. Sources: 	exttt{docs/reports/data/2026-09-10\_ra2b\_matrix.csv} (lt rows) and readout \S B4.}
\label{fig:retention}
\end{figure}
...
serve 2--13$\times$ above acquisition. The interference term is real and survives the
decay fix; Section~\ref{sec:readout} shows 83\% of it is arithmetic.
...
degrades 2--13$\times$ over acquisition. The gap lives in the readout. Three experiments,
all pre-registered, decompose it.

\begin{figure}[t]
\centering
\includegraphics[width=0.6\textwidth]{figures/f6_expansion_control.pdf}
\caption{Expansion control on the en base (one growth step; instrument gate reproduces matrix exit at 2.33 vs 2.31, PASS). Random Gaussian blocks cost $4.1\times$; inert zeros cost nothing --- the zero-init convention is load-bearing; the real ladder's joint damage is 83\% arithmetic.}
\label{fig:expansion}
\end{figure}

\subsection{The random-expansion control: 83\% is arithmetic}
...
real damage} (log scale): learned competition is real but second-order ($\sim\!17\%$).
...
\bottomrule
\end{tabular}\\
\footnotesize $^\dagger$log-scale change relative to identity; negative $=$ worse.
\end{center}
...
log-norm returned identity values to within 0.1\%).
...
input-independent by definition. The only operation class that works is
...
serving lands within $\le 8\%$ of acquisition exit quality across all domains.
...
cumulative-prefix NLL surface is \textbf{not unimodal}---multiple widths can locally
minimize the score, and the wrong local minimum wins for 5--6 languages. The failed
...
under a \textbf{class-balanced fit}, agreement is $1.000$ (Wilson $[0.977, 1.000]$) on all
160 held-out crops, at $\sim$4--8\,KB calibration text per domain. The cost is one
...
(76\% agreement, four Latin-neighbor domains at zero) was withdrawn by its author after
...
catastrophic (1614--4204) that even maximally foreign territory is consistently ``less
...
(2.36--6.47). Cross-script unseen languages sit at 82--732 (zh 417, ja 571, hi 732,
...
thresholds are empirical for this checkpoint and instrument; we state them as a
measured rule, not an architectural constant, and the conformal calibration that would
freeze them non-arbitrarily is future work (Section~\ref{sec:limits}).
...
as the only high-byte territories). We now test the growth--selection--addressing stack at
...
\caption{Cross-script routing on the fixed-regime chain (40 crops per probe). zh 37/40, ja 40/40, hi 40/40 and the decontaminated iu 40/40 concentrate on the two territories with by far the densest high-byte training exposure (bg/el); no Latin-route attraction remains after the iu correction. No linguistic kinship exists between any probe and Cyrillic or Greek.}
\label{fig:xscript}
\end{figure}
...
concentrate on the two territories with by far the densest high-byte ($>$0x80) training exposure---
...
its actual Latin-script (Turkish subtitle) content, and was eliminated by cleaning.
...
(MultiUN): best-val perplexity \textbf{2.69} (test 2.48) at the same protocol as the
...
source plus the measured rejection geometry.
...
replay answers a question growth does not need to ask.
...
\textbf{DeepSeek-V4.1-Flash Engram} (2026;\ cite as vendor documentation until
...
at the bit level; serving degrades for reasons we measured to be 83\% arithmetic and
...
\bibitem{hsp} D.~K.~M.~Hernandez, J.~Schwartz, E.~G.~Mitchell. Related mask-allocation
work (HSP). arXiv preprint (2021).
...
\bibitem{mas} D.~M.~Aljundi, R.~C.~F.~Tuytelaars, T.~Tuytelaars. \emph{Memory Aware
...
\bibitem{tinystories} E.~Eldan, Y.~Li. \emph{TinyStories: How Small Can Language Models Be
and Still Speak Coherent English?} arXiv:2305.07759 (2023).

\bibitem{tinyladder} Y.~Li, et~al. \emph{TinyLlama: An Open-Source Small Language
Model.} arXiv:2401.02395 (2024).

\bibitem{multilingual-cl} M.~de~Masson~d'Autume, et~al. \emph{Continual Learning for
Machine Translation.} arXiv preprint (2019); and related multilingual CL work.

\bibitem{l2p} Z.~Feng, et~al. \emph{Continual Learning with Pre-trained Gradual
Prompts (L2P).} ICLR (2023).

\bibitem{twist} P.~Liu, et~al. \emph{TWIST: Self-Supervised Learning of Task-Specific
Prefixes.} arXiv preprint (2023).
...
project tracker, thread 8818 (silent expert death observation).

\bibitem{engram} DeepSeek. \emph{DeepSeek-V4.1-Flash: Pushing the Limits of KV Cache
Compression.} Hugging Face model card and technical report (2026).

\bibitem{europarl} P.~Koehn. \emph{Europarl: A Parallel Corpus for Statistical Machine
Translation.} MT Summit (2005).

\bibitem{multiun} A.~R.~A.~R.~T.~Chen. \emph{MultiUN: A Multilingual Corpus from
United Nations Documents.} LREC (2012).
