...
**Datum:** 2026-08-23 · **Version:** v0.2 (Priorisierung + Plattform nach Feedback) · **Ledger:** `.jspace/` im Projektroot · **Plattform:** Nutzer-Fork, Parity-Phasen 1–6, MIT
...
- **Modelle:** BDH 10M–50M auf dem **Nutzer-Fork** (pipeline/-Struktur; Parity-Phasen 1–6 gemergt, Commit-Kette eef5f7e→067fe9f; Validierungstests laufen). Char-Level UTF-8, Regime wie Paper-Anhang (AdamW 1e-3, TBPTT 2048).
- **Fork-Synergien:** Phase 1 (State-Carry + TBPTT + sequentielle Streams) = Wake-Substrat; Phase 2 (`--no-bptt`) = fertige Totwasser-Kontrollbedingung für H4; Phase 4 (Merge-CLI) = Mechanismus F; Phase 5 (Sparsity-Logging + Synapse-Readout) = Metrik-Suite (§4).
...
**Priorität mit Gate:** H1 → H2/H3 → **[Gate: H4 nur bei grünem H2+H3]** → H4 → H5 → H6/H7.
Begründung: Ein H4-Nullresult auf kaputtem B/C wäre konfundiert („Replay unnötig“ vs. „Konsolidierung schreibt nichts“). H4 bleibt Herzstück der These, nicht der Reihenfolge.
...
4. ~~Lizenz-Check~~ **erledigt:** MIT-Lizenz bestätigt; Ableitung frei.

## 7. Verbindungen

- **Pipeline-Task:** entfällt als separater Task — der Fork ist Phase 0+1 (Parity-Phasen 1–6 abgeschlossen).
- **Fork-Follow-up „per-edge u(i,j)“:** identischer Arbeitsgegenstand wie Mechanismus E (Kaskade braucht per-Edge-Damping); die aktuelle ALiBi-Uniform-Rate ist die Übergangslösung.
